# /Users/buddy/Desktop/traderie/scripts/extract_raw_trades.py
...
def extract_file(segment, raw_path):
    ...
    for offer_rune, offer_listings in listings.items():
        ...
        for listing in offer_listings:
            ...
            try:
                ...
                num_asks = len(requested)

                normalized.append((trade_id, offered_str, requested_str, num_asks))
    ...
    with open(output_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["TradeID", "Offered", "Requested", "NumAsks"])
        writer.writerows(normalized)

    # Per segment summary
    print(f"\nSummary for {segment}:")
    print(f"  Total raw listings: {total_raw}")
    print(f"  Listings processed: {stats['total_listings']}")
    ...
    print(f"    Invalid listings: {stats['invalid']}")
...
def main():
    ...
    for segment, raw_file in RAW_FILES.items():
        segment_stats = extract_file(segment, raw_file)
        for k, v in segment_stats.items():
            total_stats[k] += v

    # Aggregate summary
    print("\n==== Total Summary Across All Segments ====")
    print(f"  Total raw listings: {total_stats['total_raw']}")
    print(f"  Listings processed: {total_stats['total_listings']}")
    print(f"  Normalized trades extracted: {total_stats['single_item'] + total_stats['and_trades']}")
    print(f"    Single-item trades: {total_stats['single_item']}")
    print(f"    AND trades: {total_stats['and_trades']}")
    print(f"  Skipped:")
    print(f"    Self-trades: {total_stats['self_trades']}")
    print(f"    Invalid listings: {total_stats['invalid']}")
    print(f"  Exceptions: {total_stats['exceptions']}")
