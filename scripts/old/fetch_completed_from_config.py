# fetch_completed_from_config.py - Simplified retry logic with single delay
...
# === Paths ===
ITEMS_JSON_PATH = "/data/item_ids.json"
CONFIG_PATH = "/server_configs.json"
BASE_OUTPUT_DIR = "/data"
LOG_PATH = "/fetch_log.txt"

# === Configurable Delays ===
PER_ITEM_DELAY = 10     # Time to wait between item fetches (in seconds)
RETRY_DELAY = 20       # Delay before retrying a failed fetch (in seconds)
...
def fetch_listings(scraper, url, params, name):
    for attempt in range(2):  # One original + one retry
        try:
...
def main():
    ...
    for cfg in server_configs:
        ...
        output_path = os.path.join(BASE_OUTPUT_DIR, f"completed_{slug}.json")
        ...
        for cat, items in existing_data.items():
            if not isinstance(items, dict):
                continue
            existing_timestamps[cat] = {}
            for name, trades in items.items():
                if isinstance(trades, list):
                    existing_timestamps[cat][name] = {
