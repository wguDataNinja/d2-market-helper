...
Only processes Ist, Ber, and Cham runes with hardcoded item IDs.
"""
...
def fetch_with_retries(url, params, item_name):
    for attempt, delay in enumerate(RETRY_DELAYS + [0]):
        scraper = cloudscraper.create_scraper()
        try:
            res = scraper.get(url, params=params, timeout=REQUEST_TIMEOUT)
...
def sanitize_trade_entry(entry):
    ...
    for p in raw:
        ...
        if isinstance(iid, str) and iid.isdigit():
            iid = int(iid)
        name = next((k for k, v in TEST_ITEMS.items() if v == iid), None)
        if name:
...
def main():
    ...
    for cfg in servers:
        ...
        for name, iid in TEST_ITEMS.items():
            params = {
                'completed': 'true', 'auction': 'false',
                'prop_Platform': platform, 'prop_Mode': mode,
                'prop_Ladder': str(ladder).lower(), 'item': iid
            }
            log(f"Fetching trades for {name}")
            res = fetch_with_retries(API_URL, params, name)
            if not res:
                continue
            try:
